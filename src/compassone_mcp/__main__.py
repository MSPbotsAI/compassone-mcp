...
def _build_http_app(mcp, settings):
    ...
    async def health(_: Request) -> JSONResponse:
        ...
        # and must not depend on CompassOne API availability.
        return JSONResponse({"status": "ok"})

    mcp_app = mcp.streamable_http_app()  # Starlette app owning the session-manager lifespan
    mounted = (
        GatewayTokenMiddleware(mcp_app, settings)
...
def main() -> None:
    ...
    if settings.mcp_transport == "http":
        ...
        print(
            f"CompassOne MCP server listening on "
        ...
        print("CompassOne MCP server running on stdio", file=sys.stderr)
